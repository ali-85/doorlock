        ...
        if (
            ...
            if ($request->key) {
                ...
                if ($isClose->jam_Keluar) {
                    // return response()->json(['data' => $isClose->jam_Keluar]);
                    if (date('Y-m-d H:i:s') > date('Y-m-d H:i:s', strtotime($isClose->jam_Keluar. ' +1 hour'))) {
                        // Foto Absen
                        if (isset($foto)) {
                    ...
                    if (date('Y-m-d H:i:s') > date('Y-m-d H:i:s', strtotime($isClose->jam_masuk. ' +12 hour'))) {
                        if (isset($foto)) {
                            $imgname = $cekRfid->nama . '_masuk_' . date("dmY_H-i-s_", time()).uniqid(rand(0,5)).'.'. $foto->getClientOriginalExtension();
                            if(in_array($foto->getClientOriginalExtension(), array("jpg", "jpeg", "gif", "png"))){
                                $tujuan_upload = 'files/Absensi/' . Carbon::now()->format('Y').'/'.Carbon::now()->format('m');
                        ...
                        if (date('Y-m-d H:i:s') > date('Y-m-d H:i:s', strtotime($isClose->jam_masuk. ' +1 hour'))) {
                            if (isset($foto)) {
                            ...
                            $monitoringdata = OutMonitoring::where('memploye_id', $cekRfid->id)
                                ->whereNull('tmend')
                                ->orderBy('id', 'desc')
                                ->first();
                            if ($monitoringdata) {
